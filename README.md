...
- **复制链接**：盒头「复制本盒链接」按钮、工具栏「复制当前筛选链接」、盒筛选对话框与空状态均提供复制（`navigator.clipboard` + `execCommand` 回退，失败时 toast 展示 URL），成功 toast `已复制 #52`。干员级深链 `#52&op=3` / `#52&c=名字` 直达并高亮盒内角色卡片。
- **跳到盒 / 返回上一筛选**：工具栏「跳转」面板按盒号 / 盒名 / 角色名直达（Enter 首个建议）；「返回上一筛选」走内存栈回退上一盒选/type 状态（搜索 `q` 不刷历史）。键盘 `/` 聚焦搜索，筛选对话框有焦点圈闭与 Esc 还原。
- **深链基准与跨站约定**：盒 token 规则（`src/lib/boxRoutes.ts`）与 `data/box-routes.json`（`scripts/build-box-routes.mjs` 生成）是跨站单一事实源，详见 `docs/deep-links.md`；复制/分享/toast 交互范式见 `docs/copy-share-toast.md`。
- **社区别名搜索（B3）**：构建期把 `searchWord.json` 的社区别名并入搜索索引（如 阿米娅→兔兔/罗德岛CEO、极境→鸡精/大帅哥）。
- **入站主题参数（C5）**：`?theme=light|dark|system` 首载即生效，不覆盖本地设置。
...
- **`#none` 与无效 token**：`#none` 表示“一盒都不显示”的特殊状态；无法解析的 token 不再静默——`parseBoxRouteStrict` 暴露 `unknownTokens`，加载/路由变更时会 toast「链接中有无法识别的盒：…」并保持页面全量可用。
